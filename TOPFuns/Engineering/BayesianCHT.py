...
def g_fun(x: torch.Tensor, m: int, p: int, n: int) -> torch.Tensor:
    ...
    if n <= 1:  
        ...
        x_opt = 0.5 + 0.01 / (n - 1) * torch.sin(5 * torch.pi * x[:, :-1]).sum(dim=1, keepdim=True)
...
def objectivefunA(x: torch.Tensor, m: int, p: int) -> torch.Tensor:
    ...
    if m == 1:
        ...
        h_term2 += 10 * torch.sqrt(1 - f[:, :sqrt_term_cols] ** 2).sum(dim=1, keepdim=True)
...
class NonLinearConstraintProblemA3(BenchmarkProblem):
    ...
    def _evaluate_implementation(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.p >= 1:
            ...
            cm += 0.2 / self.p * torch.sin(1.5 * math.pi * theta).sum(dim=1, keepdim=True)
...
class NonLinearConstraintProblemA7(BenchmarkProblem):
    ...
    def _evaluate_implementation(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.p >= 1:  
            cm = 0.8 + 0.1 / self.p * torch.sin(2 * math.pi * x[:, :self.p]).sum(dim=1, keepdim=True)
        ...
        if self.m > 1:  
            ...
            if self.p >= 1:  
                cm += 0.1 / (n - 1) * torch.cos(4 * math.pi * x[:, :self.p]).sum(dim=1, keepdim=True)
                cm2 -= 0.1 / (n - 1) * torch.cos(4 * math.pi * x[:, :self.p]).sum(dim=1, keepdim=True)
            ...
            cm += 0.1 / (n - 1) * torch.cos(4 * math.pi * fx[:, :cols]).sum(dim=1, keepdim=True)
            cm2 -= 0.1 / (n - 1) * torch.cos(4 * math.pi * fx[:, :cols]).sum(dim=1, keepdim=True)
        ...
        gix = torch.cat(gix, dim=1) if gix else torch.empty((x.shape[0], 0), device=x.device, dtype=x.dtype)
...
class NonLinearConstraintProblemA8(BenchmarkProblem):
    ...
    def _evaluate_implementation(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.p >= 1:  
            cm_old += 0.3 / self.p * torch.sin(1.5 * math.pi * x[:, :self.p]).sum(dim=1, keepdim=True)

            if self.m > 1:
                f = objectivefunA(x, self.m, self.p)
                
                f_cols = min(self.m - 1, f.shape[1])
                cm_old2 += (0.2 * torch.sin(math.pi * x[:, :1]) / (self.m - 1) * torch.cos(math.pi / 2 * f[:, :f_cols])).sum(dim=1, keepdim=True)
        ...
        if self.m > 1:  
            ...
            f_cols = min(self.m-1, f.shape[1])
            cm += 0.1 / (n - 1) * torch.cos(4 * math.pi * f[:, :f_cols]).sum(dim=1, keepdim=True)
            cm2 -= 0.1 / (n - 1) * torch.cos(4 * math.pi * f[:, :f_cols]).sum(dim=1, keepdim=True)
        ...
        gix = torch.cat(gix_list, dim=1) if gix_list else torch.empty((x.shape[0], 0), device=x.device, dtype=x.dtype)
...
class NonLinearConstraintProblemB3(BenchmarkProblem):
    ...
    def __init__(self,
            ...
            bounds=None,
            num_constraints: int = 0,):
        super().__init__(dim=dim, num_objectives=num_objectives, num_constraints=num_constraints, bounds=bounds)
        self.p = dim
        self.m = num_objectives
    ...
    def _evaluate_implementation(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.p >= 1:
            theta = parameterfunA(x, self.p)
            cm += (0.3 / self.p) * torch.sin(1.5 * torch.pi * theta).sum(dim=1, keepdim=True)
            gix = -(x[:, self.p:self.p+1] - cm) 
        else:
            
            fx_cols = min(self.m - 1, fx.shape[1])
            cm += (0.3 / (x.shape[1] - 1)) * torch.sin(torch.pi * (fx[:, :fx_cols] + 0.5)).sum(dim=1, keepdim=True)
...
class NonLinearConstraintProblemB4(BenchmarkProblem):
    ...
    def __init__(self,
            ...
            num_objectives: int = 1,
            bounds=None,
            num_constraints: int = 1) -> None:
        super().__init__(dim=dim, bounds=bounds, num_objectives=num_objectives, num_constraints=num_constraints)
        self.p = dim
        self.m = num_objectives
    ...
    def _evaluate_implementation(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.p >= 1:
            theta = parameterfunA(x, self.p)
            cm += (0.1 / self.p * torch.sin(2 * torch.pi * theta)).sum(dim=1, keepdim=True)
            ...
            f_cols = min(self.m-1, f.shape[1])
            cm += 0.2 / (n - 1) * torch.sin(torch.pi * (f[:, :f_cols] + 0.5)).sum(dim=1, keepdim=True)
...
class NonLinearConstraintProblemB7(BenchmarkProblem):
    ...
    def _evaluate_implementation(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if m > 1:
            ...
            if p >= 1:
                cm += 0.15 / (n - 1) * torch.sin(10 * torch.pi * x[:, :p]).sum(dim=1, keepdim=True)
                cm2 -= 0.15 / (n - 1) * torch.sin(10 * torch.pi * x[:, :p]).sum(dim=1, keepdim=True)

            f = objectivefunB(x, m, p) 
            
            f_cols = min(m-1, f.shape[1])
            cm += (0.05 / (n - 1) * torch.sin(10 * torch.pi * f[:, :f_cols])).sum(dim=1, keepdim=True)
            cm2 -= (0.05 / (n - 1) * torch.sin(10 * torch.pi * f[:, :f_cols])).sum(dim=1, keepdim=True)
            ...
            gix.append(cm2 - x[:, last_index:last_index+1])
             ...
             cm = 0.8
             if p >= 1:
                cm += 0.1 / p * torch.sin(2 * torch.pi * x[:, :p]).sum(dim=1, keepdim=True)
             last_index = min(p+m-1, x.shape[1]-1) 
             gix.append(-(x[:, last_index:last_index+1] - cm))
             f = objectivefunB(x, m, p)  
...
class NonLinearConstraintProblemB8(BenchmarkProblem):
    ...
    def _evaluate_implementation(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.p >= 1:
            cm_old += (0.3 / self.p * torch.sin(1.5 * torch.pi * x[:, :self.p])).sum(dim=1, keepdim=True)

            if self.m > 1:
                f = objectivefunB(x, self.m, self.p) 
                
                f_cols = min(self.m - 1, f.shape[1])
                cm_old2 += (0.2 * torch.sin(torch.pi * x[:, :1]) / (self.m - 1) * torch.cos(torch.pi / 2 * f[:, :f_cols])).sum(dim=1, keepdim=True)
        ...
        if self.m > 1:
            ...
            if self.p >= 1:
                cm += (0.1 / (n - 1) * torch.cos(4 * torch.pi * x[:, :self.p])).sum(dim=1, keepdim=True)
                cm2 -= (0.1 / (n - 1) * torch.cos(4 * torch.pi * x[:, :self.p])).sum(dim=1, keepdim=True)

            
            f_cols = min(self.m-1, f.shape[1])
            cm += (0.1 / (n - 1) * torch.cos(4 * torch.pi * f[:, :f_cols])).sum(dim=1, keepdim=True)
            cm2 -= (0.1 / (n - 1) * torch.cos(4 * torch.pi * f[:, :f_cols])).sum(dim=1, keepdim=True)
        ...
        gix_list = [gix1, gix2, gix3, gix4]
        gix_list = [t for t in gix_list if t.numel() > 0]  
        gix = torch.cat(gix_list, dim=1) if gix_list else torch.empty((batch_size, 0), device=x.device, dtype=x.dtype)
