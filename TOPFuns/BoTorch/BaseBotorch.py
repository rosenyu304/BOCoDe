...
class BotorchProblem(BenchmarkProblem):
    ...
    def __init__(self, botorch_problem, optimum=None, x_opt=None, dim=None):
        ...
        num_cons = self.botorch_problem.num_constraints if hasattr(self.botorch_problem, 'num_constraints') else 0
    ...
    def _evaluate_implementation(self, X: torch.Tensor, scaling = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # if self.fixedDim:
        if self.num_constraints is not 0:
            return self.botorch_problem.evaluate_slack_true(X), self.botorch_problem(X).unsqueeze(-1)
