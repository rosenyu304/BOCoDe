"""TFM-CqNEHVI: constrained qNEHVI on a tabular-foundation-model surrogate.

Constrained qNEHVI exactly as in ``multi_obj_constrained/constrained_qnehvi`` -- the
hypervolume improvement is only credited for points predicted to satisfy every
constraint (``c <= 0``) -- but the GPs are replaced by a **frozen, pretrained TabPFN**.
...
BoTorch multi-output ``Model``, so BoTorch's stock
...
uses. Each output is conditioned and standardized independently. As in the GP version, an
``IdentityMCMultiOutputObjective`` selects the objective outputs ``0..m-1`` and the
constraint outputs ``m..m+n_c-1`` are passed as ``constraints=`` callables (BoTorch's
convention -- feasible when the callable returns ``<= 0`` -- matches BoCoDe's ``c <= 0``,
so the constraints are modeled raw, with no negation anywhere).
...
Needs TabPFN >= v3 (see docs/tfm_setup.md). Run::
    ...
    python -m algorithms.multi_obj_constrained.tfm_cqnehvi --problem WeldedBeam --init 12 --iters 50
...
def optimize_problem(
    ...
    """TFM-CqNEHVI constrained multi-objective BO over the unit cube.
    ...
    assert nc > 0, "tfm_cqnehvi requires a constrained problem"
    if n_init is None:
    ...
    # Constraints are model outputs m..m+nc-1; BoTorch convention is feasible when the
    # constraint callable returns <= 0, matching BoCoDe's c <= 0 (no negation).
    ...
    for it in range(start_it, iters):
        ...
        model = TabPFNModel(surrogate, train_X.to(dev), outputs)
