"""TabICL-TuRBO: TuRBO with a tabular foundation model (TabICL) as the local surrogate.
...
but with the local **GP replaced by a frozen, pretrained TabICL regressor**: the trust
region's observed points condition the model in-context and the candidates are scored by
Thompson sampling from its predictive. This is the TabICL counterpart of ``tfm_turbo``.
...
surrogate is a TFM?"). One thing TuRBO gets from a GP has no TabICL counterpart, and the
substitute is **our design choice**:

1. **Thompson sampling.** TuRBO's acquisition draws a realization of the GP posterior over
   the candidate set and takes its argmax (``MaxPosteriorSampling``). TabICL gives a
   *marginal* predictive distribution per row and no joint posterior, so there is no
   correlated sample path to draw. We draw **independent samples from each candidate's own
   predictive** (``TabICLSurrogate.sample_from_quantiles``, an inverse-CDF draw on the
   predictive's quantile grid) and take the argmax. This is Thompson sampling under an
   independence assumption; it is noisier / more exploratory than a GP's correlated draw,
   and that difference is a property of the surrogate, not a bug.

The trust region is an **isotropic cube** (``x_center +/- L/2``). TuRBO scales side length
``i`` by the GP's ``i``-th ARD lengthscale; TabICL has no lengthscales, so there is nothing
to weight the sides by -- the same choice tfm_turbo / tfm_scbo make.

Needs TabICL (``pip install tabicl``). Run::

    python -m algorithms.single_obj.tabicl_turbo --problem Ackley --init 20 --iters 50
...
def _generate_batch(
    ...
    """TuRBO's candidate set inside the trust region, scored by TabICL Thompson sampling."""
    dim = X.shape[-1]
    x_center = X[Y.argmax(), :].clone()
    # Isotropic trust region (a cube): ``x_center +/- L/2``. TuRBO weights each side by its
    # GP's ARD lengthscale; TabICL has no lengthscales, so the region is a cube -- the same
    # honest choice tfm_turbo / tfm_scbo make.
    half = tr.length / 2.0
    ...
    # Thompson sampling on TabICL's predictive: one independent draw per candidate, argmax;
    # repeat without replacement for a batch (TuRBO's MaxPosteriorSampling does the same, but
    # from a *correlated* GP sample path -- see the module docstring).
    with torch.no_grad():
        surrogate.fit(X, Y)
        _, _, q = surrogate.score(X_cand)
        chosen: list[int] = []
        for _ in range(batch_size):
            draw = surrogate.sample_from_quantiles(q).reshape(-1).to("cpu")
            if chosen:
...
def optimize_problem(
    ...
    """TabICL-TuRBO over the unit cube. ``n_init`` defaults to the dim-scaled BoCoDe default.

    The trust-region bookkeeping is TuRBO's (restarts included: a restart's initial design
    consumes evaluations from ``iters``); only the surrogate differs (TabICL Thompson
    sampling in place of the GP), and the trust region is an isotropic cube because TabICL
    ...
    res = Result(
        ...
        seed,
        acquisition_function="TabICL predictive Thompson sampling (isotropic trust region)",
    )
