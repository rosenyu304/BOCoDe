"""TFM-qNEHVI: qNEHVI on a tabular-foundation-model surrogate.

qNEHVI (Daulton et al., 2021) exactly as in ``multi_obj/qnehvi``, but the GPs are
replaced by a **frozen, pretrained TabPFN**. There is no model fitting: TabPFN
conditions on the observed data in-context at every acquisition evaluation.

The surrogate is :class:`algorithms._tfm_utils.TabPFNModel`, which dresses TabPFN up
as a BoTorch multi-output ``Model``, so BoTorch's stock
``qLogNoisyExpectedHypervolumeImprovement`` and ``optimize_acqf`` run on it unchanged
(gradients flow through the network to the candidate).
...
The posterior is a ``GPyTorchPosterior`` whose mean and variance are the first two
moments of TabPFN's bar distribution, with a **diagonal** covariance -- a PFN gives
marginal predictives, never a joint covariance across query points. qNEHVI stays a valid
MC estimator, it just sees no cross-point correlation; the acquisition is therefore built
with ``cache_root=False`` (the cached-Cholesky path exists to exploit a joint GP
covariance this model does not have). See :class:`TabPFNModel` for the full statement of
the two approximations.

Needs TabPFN >= v3 (see docs/tfm_setup.md). Run::

    python -m algorithms.multi_obj.tfm_qnehvi --problem RE24 --init 10 --iters 50
...
def optimize_problem(
    ...
    """TFM-qNEHVI multi-objective BO over the unit cube.
    ...
    res = Result(
        "tfm_qnehvi", type(problem).__name__, seed, acquisition_function="qLogNEHVI"
    )
    ...
    for it in range(start_it, iters):
        ...
        model = TabPFNModel(surrogate, train_X.to(dev), train_Y.to(dev))
