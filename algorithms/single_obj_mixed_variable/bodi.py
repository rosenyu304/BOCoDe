...
class _Space:
    ...
    def __init__(self, problem):
        ...
        self.grids = grids
        self.disc_dims = sorted(
            grids
        )  # positions of discrete (integer/categorical) dims
...
def optimize_problem(
    ...
    resolve_device(
        device
    )  # honored for parity; the embedding GP is tiny and runs on CPU
    ...
    n_alt = (
        N_ALT if space.n_cont else 1
    )  # no continuous block -> one local search suffices
