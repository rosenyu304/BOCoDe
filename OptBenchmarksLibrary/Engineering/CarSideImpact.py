...
class CarSideImpact(BenchmarkProblem):
    ...
    def evaluate(self, X, to_verify = True):
        ...
        gx[:, 2] = 0.214 + 0.00817 * x5 − 0.045195 * x1 − 0.0135168 * x1 + 0.03099 * x2 * x6 − 0.018 * x2 * x7 + 0.007176 * x3 + 0.023232x3 − 0.00364 * x5 * x6 − 0.018 * x2**2 - 0.32
        gx[:, 3] = 0.74 − 0.61 * x2 − 0.031296 * x3 − 0.031872 * x7 + 0.227 * x2**2 - 0.32
        gx[:, 4] = 28.98 + 3.818 * x3 − 4.2 * x1 * x2 + 1.27296 * x6 − 2.68065 * x7 - 32
        gx[:, 5] = 33.86 + 2.95 * x3 − 5.057 * x1 * x2 − 3.795 * x2 − 3.4431 * x7 + 1.45728 - 32
