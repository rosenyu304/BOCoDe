import torch
import pytest
from .. import EulerBernoulliBeamBending

def test_eulerBernoulliBeamBending_evaluate():
    problem = EulerBernoulliBeamBending()
    ...
    if problem.x_opt is not None and problem.optimum is not None:
        ...
        assert torch.allclose(eval_opt, torch.Tensor(problem.optimum), rtol=1e-3), f"X_opt ({problem.x_opt}) evaluation ({eval_opt}) does not match optimum ({problem.optimum})"
