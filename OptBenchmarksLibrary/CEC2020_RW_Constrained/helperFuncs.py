...
def OBJ11(x, n):
    ...
    def fhd(z):
        term1 = np.arccos((a**2 + (l - z)**2 + e**2 - b**2) / (2 * a * np.sqrt((l - z)**2 + e**2)))
        term2 = np.arccos((b**2 + (l - z)**2 + e**2 - a**2) / (2 * b * np.sqrt((l - z)**2 + e**2)))
...
def ConsBar10(x):
    ...
    gcoord = np.array([[18.288, 18.288, 9.144, 9.144, 0, 0],
                        [9.144, 0, 9.144, 0, 9.144, 0]])
    
    element = np.array([[3, 1, 4, 2, 3, 1, 4, 3, 2, 1],
                         [5, 3, 6, 4, 4, 2, 5, 6, 3, 4]])
...
def Cal_K_and_M(type_, gcoord, element, A, rho, E):
    ...
    for iel in range(nel):
        ...
        Te = np.array([[l_ij, m_ij, 0, 0],
                       [0, 0, l_ij, m_ij]])
        
        ke = (A[iel] * E / le) * np.array([[1, -1],
                                           [-1, 1]])
        ke = Te.T @ ke @ Te
        
        me = (rho * le * A[iel] / 6) * np.array([[2, 0, 1, 0],
                                                 [0, 2, 0, 1],
                                                 [1, 0, 2, 0],
                                                 [0, 1, 0, 2]])
...
def downstream_wind_turbine_is_affected(coordinate, upstream, downstream, theta, kappa, R):
...
def eva_func_deficit_caching(interval_dir_num, N, coordinate, theta, a, kappa, R):
    ...
    for i in range(N):
        ...
        if i != movedTurbine:
            ...
            vel_def_i = (np.sum(thetaVeldefijMatrix[i, :, idx]**2)
...
def eva_power(interval_dir_num, interval_dir, N, coordinate, a, kappa, R, k_val, c_val,
              cut_in_speed, rated_speed, cut_out_speed, evaluate_method):
    ...
    for i in range(N):
        for j in range(1, n_ws + 1):
            ...
            term1 = 1500 * math.exp(((v_j_1 + v_j) / 2) - 7.5) / (5 + math.exp(((v_j_1 + v_j) / 2) - 7.5))
            term2 = math.exp(- (v_j_1 / interval_c[i])**k_val) - math.exp(- (v_j / interval_c[i])**k_val)
            power_eva[i] += term1 * term2
        power_eva[i] += 1500 * (math.exp(- (rated_speed / interval_c[i])**k_val) - math.exp(- (cut_out_speed / interval_c[i])**k_val))
    return power_eva


def Fitness(interval_num, interval, fre, N, coordinate, a, kappa, R, k_array, c_array,
            cut_in_speed, rated_speed, cut_out_speed, evaluate_method):
    all_power = 0
    for i in range(interval_num):
        ...
        power_eva = eva_power(interval_dir_num, interval_dir, N, coordinate, a, kappa, R,
                              k_array[i], c_array[i], cut_in_speed, rated_speed, cut_out_speed, evaluate_method)
