import torch
import pytest
from .. import BBOB, BBOB_Biobj, BBOB_BiobjMixInt, BBOB_Boxed, BBOB_Constrained, BBOB_LargeScale, BBOB_MixInt, BBOB_Noisy
import cocoex
import random
import warnings

suiteClasses = [BBOB, BBOB_Biobj, BBOB_BiobjMixInt, BBOB_Boxed, BBOB_Constrained, BBOB_LargeScale, BBOB_MixInt, BBOB_Noisy]
suiteNames = ["bbob", "bbob-biobj", "bbob-biobj-mixint", "bbob-boxed", "bbob-constrained", "bbob-largescale", "bbob-mixint", "bbob-noisy"]
...
# Total 2072 items
@pytest.mark.parametrize("suite, function_number, instance_number, dimension", generate_test_cases())
def test_BBOB_evaluate(suite, function_number, instance_number, dimension):
    try:
        problem = suite(dim=dimension, function_number=function_number, instance_number=instance_number)
    except cocoex.exceptions.NoSuchProblemException as e:
        print(f"Skipping {suite.__name__}({dimension}, {function_number}, {instance_number}) due to NoSuchProblemException")
        return
