...
def integrate(x, y, lbnd, ubnd):
    ...
    if compl_ind.size == 0:  # if both bounds fall within an interval
        ...
        y_lbnd_approx = (lbnd - x[idx - 1]) / (x[idx] - x[idx - 1]) * (
            y[idx] - y[idx - 1]
        ) + y[idx - 1]
        y_ubnd_approx = (ubnd - x[idx - 1]) / (x[idx] - x[idx - 1]) * (
            y[idx] - y[idx - 1]
        ) + y[idx - 1]
    ...
    lidx = compl_ind.min()
    y_lbnd_approx = (lbnd - x[lidx - 1]) / (x[lidx] - x[lidx - 1]) * (
        y[lidx] - y[lidx - 1]
    ) + y[lidx - 1]
    ...
    y_ubnd_approx = (ubnd - x[uidx - 1]) / (x[uidx] - x[uidx - 1]) * (
        y[uidx] - y[uidx - 1]
    ) + y[uidx - 1]
...
def integrate_sq(x, y, lbnd, ubnd):
    ...
    if compl_ind.size == 0:  # if both bounds fall within an interval
        ...
        return (
            m**2 / 3 * (ubnd**3 - lbnd**3)
            + (m * y_lbnd_approx - m**2 * lbnd) * (ubnd**2 - lbnd**2)
    ...
    xis = x[compl_ind[:-1]]
    yis = y[compl_ind[:-1]]
    full_blocks_integral = (
        ms**2 * dx3 / 3 + (ms * yis - ms**2 * xis) * dx2 + (ms * xis - yis) ** 2 * dx
    ).sum()
    ...
    lower_block_integral = (
        m**2 / 3 * (x[lidx] ** 3 - lbnd**3)
    ...
    # integrate upper hanging partial block
    uidx = compl_ind.max()
    y_ubnd_approx = (ubnd - x[uidx]) / (x[uidx + 1] - x[uidx]) * (
        y[uidx + 1] - y[uidx]
    ) + y[uidx]
    m = (y_ubnd_approx - y[uidx]) / (ubnd - x[uidx])
    upper_block_integral = (
        m**2 / 3 * (ubnd**3 - x[uidx] ** 3)
...
def calc_dzetatildes(thetas, k, cangles, alphas, jmfs):
    ...
    dgamma = -jmfs[k - 1](thetas[k - 1] + cangles[k - 1] / 2) + jmfs[k - 1](
        thetas[k - 1] - cangles[k - 1] / 2
    )
...
class ReactivityModel:
    ...
    def __init__(self, typ="wtd"):  # abs, wtd or refl
        ...
        self.cangles = np.array([130, 145, 145, 130, 130, 145, 145, 130]) / 180 * np.pi
    ...
    def evald(self, pert, k, zetatildes=None):
        ...
        dzetatildes = calc_dzetatildes(
            thetas=pert, k=k, cangles=self.cangles, alphas=self.alphas, jmfs=self.jmfs
        )
        ...
        drdtk[k - 1] += zetatildes[k - 1] * calc_W(
            pert[k - 1], self.cangles[k - 1], self.jmfs[k - 1]
        )
...
if __name__ == "__main__":
    a = ReactivityModel()
    p5 = np.array(
        [0, 0, 2.201331, -2.137274, -2.512146, 2.02961, 0.19566370000000002, 3.140272]
    )
    print(a.eval(p5))
