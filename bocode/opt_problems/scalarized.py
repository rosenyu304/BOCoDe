...
_OBJ_SCALE = {
    "Mazda": [
        0.043776470595867735,
        0.9755064854862865,
        0.02844620171752701,
        0.026475858114910756,
        0.02404480081325757,
    ],
    "Mazda_SCA": [
        0.03616851590877216,
        2.462419148828268,
        0.025541939770907903,
        0.025417065644670056,
    ],
...
class PenaltyScalarizedProblem(BenchmarkProblem):
    ...
    def _scalarize(self, base_values, base_cons):
        """Penalty-scalarize the base's (values, constraints) into one SO value.

        Normalize each objective by its fixed per-objective scale before the
        equal-weight mean so no single large-magnitude objective dominates, then
        subtract the summed constraint violation (feasible when <= 0).
        """
    ...
    def evaluate(self, X):
        ...
        return self._scalarize(base_values, base_cons), None

    def _evaluate_implementation(self, X, scaling: bool = False):
        # Standard BenchmarkProblem entry point (returns ``(constraints, values)``).
        # The BO campaign goes through the ``evaluate`` override above; this exists so
        # the direct-call convention (e.g. the smoke test) works too. ``scaling=True``
        # means X arrives in the unit cube and must be mapped to the base's bounds.
        if scaling:
            X = self.scale(X)
        base_values, base_cons = self._base.evaluate(X)
        return None, self._scalarize(base_values, base_cons)
