...
class NormalizedInputFn:
    ...
    def get_range(self):
        return np.array(
            [np.zeros(self.x_range[0].shape[0]), np.ones(self.x_range[0].shape[0])]
        )
...
class GMCost:
    ...
    def __call__(self, X):
        ...
        return np.exp(
            -np.sum(
                ((X[:, :, None] - self.c.T[None, :, :]) / self.s.T[None, :, :]) ** 2,
                axis=1,
            )
        ).dot(self.w)
...
def plot_2d_rover(
    ...
    traj_points = roverdomain.traj.get_points(
        np.linspace(0.0, 1.0, ntraj_points, endpoint=True)
    )
...
def generate_verts(rectangles):
    ...
    for l, h in zip(rectangles.l, rectangles.h):
        ...
        plist = [
            [verts[vert_ind[ix][iy]] for iy in range(len(vert_ind[0]))]
            for ix in range(len(vert_ind))
        ]
        faces = [
            [verts[faces[ix][iy]] for iy in range(len(faces[0]))]
...
def plot_3d_forest_rover(roverdomain, rectangles, ntraj_points=100):
    ...
    # plot rectangles
    ax.add_collection3d(
        Poly3DCollection(poly3d, facecolors=(0.7, 0.7, 0.7, 1.0), linewidth=0.5)
    )
