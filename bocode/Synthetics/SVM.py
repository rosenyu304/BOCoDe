...
class SVM(BenchmarkProblem):
    ...
    def __call__(self, x: np.array, verbose):
        ...
        svr = SVR(gamma=gamma, epsilon=epsilon, C=C, cache_size=1500, tol=0.001, verbose=verbose)
    ...
    def _evaluate_implementation(self, x: torch.Tensor, scaling=False, verbose=False) -> torch.Tensor:
